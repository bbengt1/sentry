...
- [ ] **WIZ-01**: Maker can run a Live Preview calibration wizard to collect samples and stage a fit before commit
- [ ] **WIZ-02**: Maker can Apply or Cancel calibration; Cancel leaves no calibrated state or meter claims
- [ ] **WIZ-03**: Wizard shows clear feedback (sample count, residual/status, calibrated vs relative labeling)
- [ ] **WIZ-04**: Draft/staging mode never claims `metric_calibrated` on the live perception stream until Apply

### Free-space metric path

- [ ] **FS-01**: Free-space products use `units="m"` only when underlying depth is `metric_calibrated`
- [ ] **FS-02**: Free-space does not flip unit labels while still computing pure ordinal percentile nearness as if meters
- [ ] **FS-03**: Free-space smoother/state resets on calibration apply and clear so stale ordinal/metric state does not ghost

### Persistence

- [ ] **PER-01**: Maker can save calibration keyed by `camera_id` (and fingerprint fields needed for safety)
- [ ] **PER-02**: On `sentry serve`, valid saved calibration can re-apply for matching camera/fingerprint without re-running the wizard
- [ ] **PER-03**: Mismatched fingerprint (resolution, model/mode, camera_id) refuses auto-apply and keeps honest relative depth
- [ ] **PER-04**: Maker can clear/invalidate stored calibration and return to uncalibrated relative depth

### Operator surfaces

- [ ] **OPS-01**: Status/banner (and Live Preview) show whether calibration is active and base honesty (relative vs calibrated)
- [ ] **OPS-02**: Operator docs describe the calibration wizard, persistence path, and honesty rules (no vehicle-grade claims)
- [ ] **OPS-03**: Automated tests cover fit/apply/honesty/persist with synthetic data (no physical room required in CI)
