...
def test_publish_returns_same_frame_identity(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
    frame = image_frame_factory(frame_id=7)
    bus.publish(frame)
...
def test_second_publish_increments_frames_dropped(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
    bus.publish(image_frame_factory(frame_id=0))
...
def test_depth_one_only_latest_retained_after_many_publishes(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
    for i in range(50):
        bus.publish(image_frame_factory(frame_id=i))
    latest = bus.get_latest()
    assert latest is not None
...
def test_frames_published_counts_every_publish(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
    for i in range(5):
...
def test_metrics_snapshot_is_isolated_copy(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
    bus.publish(image_frame_factory(frame_id=0))
...
def test_get_latest_does_not_consume(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    """Keep-latest is not consume-once: repeated get returns same slot."""
    bus = FrameBus()
    frame = image_frame_factory(frame_id=3)
    bus.publish(frame)
    a = bus.get_latest()
    b = bus.get_latest()
    assert a is frame
    ...
    bus.publish(image_frame_factory(frame_id=4))
    assert bus.metrics_snapshot().frames_dropped == 1


def test_capture_fps_non_negative_after_publishes(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
    for i in range(10):
...
def test_capture_fps_updates_after_one_second_window(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    ...
    while time.monotonic() < deadline:
        bus.publish(image_frame_factory(frame_id=n))
        n += 1
        time.sleep(0.02)
...
def test_concurrent_publish_get_latest_smoke(
    image_frame_factory: Callable[..., ImageFrame],
) -> None:
    bus = FrameBus()
